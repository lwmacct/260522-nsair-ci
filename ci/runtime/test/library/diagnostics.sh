...
__container_logs() {
  ...
  docker logs --tail "${NSAIR_CI_LOG_TAIL:-60}" "$_name" >&2 || true
}
...
__dump_debug() {
  ...
  echo "nsair runtime validation failed with exit code $_code" >&2
  ...
  tail -260 /var/log/nsair-daemon.log 2>/dev/null |
